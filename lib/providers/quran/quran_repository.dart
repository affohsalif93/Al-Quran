...
import 'package:quran/utils/download_image_data.dart';
...
final quranRepositoryProvider = FutureProvider<QuranRepository>((ref) async {
  ...
  final dbFile = join(currentDir, 'data', 'dbs', "black-images-word-by-word.db");
  logger.fine('Database file path: $dbFile');
  if (!await File(dbFile).exists()) {
    logger.error('Database file does not exist: $dbFile');
  }

  final db = await openDatabase(dbFile, readOnly: false);

  await downloadMissingImages(db, batchSize: 80, concurrency: 32);
